...
> `2_nandha_engines/samples/inputs/{currents,wind}.nc` if you want to diff against it.
...
> (`2_nandha_engines/benchmark/`), and it reports 86% top-1 / 100% top-3. When yours is
...
> three: `cd 2_nandha_engines && python scripts/run_all.py` (~3.5 s, no network, no GPU).
...
>    four handbooks under `2_nandha_engines/docs/` are **untracked**. Same for anyone
