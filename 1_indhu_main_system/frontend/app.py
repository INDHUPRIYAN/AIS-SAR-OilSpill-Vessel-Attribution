...
def build_map(run: dict, show: dict):
    ...
    folium.TileLayer("cartodbpositron", name="Basemap (light)").add_to(m)
    folium.TileLayer("cartodbdark_matter", name="Basemap (dark)", show=False).add_to(m)
    ...
    if st.button("Run pipeline", type="primary", use_container_width=True):
        ...
        with st.spinner(f"Running {run_id}…"):
            proc = subprocess.run(
                ...
                cwd=str(REPO_ROOT / "1_indhu_main_system"),
